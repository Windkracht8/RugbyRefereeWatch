...
fun ManagePlayers(
	...
	Column(Modifier
				...
				OutlinedButton(
					onClick = {
						tempPlayers.add(Player(
							number = if (tempPlayers.isEmpty()) 1 else tempPlayers.maxOf { it.number } + 1
						))
		...
		LazyColumn(
			...
			items(tempPlayers){ player -> ManagePlayersPlayer(player, tempPlayers, isEdit) }
		}
		Row {
			OutlinedButton(
				onClick = close
			) { Text(if(isEdit) R.string.cancel else R.string.close) }
			Spacer(Modifier.weight(1f))
			if(isEdit) {
				Button(
					onClick = {
	...
	if (showManagePlayersIntro && !LocalInspectionMode.current) {
		AlertDialog(
			text = {
				Text(
...
fun ManagePlayersPlayer(
	...
	Row(modifier = Modifier.fillMaxWidth()) {
		...
		TextButton(
			onClick = { if (isEdit) player.frontRow = !player.frontRow }
		){
			Text(
